...
func (o *ObjectRelationship) JoinString() string {
	...
	if o.IsManyToMany() {
		...
		join += fmt.Sprintf("\"LEFT JOIN \"+dialect.Quote(\"%[1]s\")+\" \"+dialect.Quote(_alias)+\" ON \"+dialect.Quote(_alias)+\".id = \"+alias+\".\"+dialect.Quote(\"%[2]s_id\")", o.Target().TableName(), o.Name())
	} else if o.IsToMany() {
		join += fmt.Sprintf("\"LEFT JOIN \"+dialect.Quote(\"%[1]s\")+\" \"+dialect.Quote(_alias)+\" ON \"+dialect.Quote(_alias)+\".\"+dialect.Quote(\"%[3]s_id\")+\" = \"+dialect.Quote(alias)+\".id\"", o.Target().TableName(), o.Name(), o.InverseRelationshipName())
	}
	return join
}
