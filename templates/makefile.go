...
run:
	DATABASE_URL=mysql://'root:123456@localhost:3306/graphql?charset=utf8mb4&parseTime=True&loc=Local' PORT=80 go run *.go
...
test:
	go build -o app *.go && (DATABASE_URL=mysql://'root:123456@localhost:3306/graphql?charset=utf8mb4&parseTime=True&loc=Local' PORT=80 ./app& export app_pid=$$! && make test-godog || test_result=$$? && kill $$app_pid && exit $$test_result)
